...
float * ofSoundGetSpectrum(int nBands){
	...
	if (nBands > 8192){
		...
		cerr << "error in ofSoundGetSpectrum, the maximum number of bands is 8192 - you asked for "<< nBands <<" we will return 8192" << endl;
		nBands = 8192;
	} else if (nBands <= 0){
		//ofLog(OF_LOG_ERROR, "error in ofSoundSpectrum, you've asked for %i bands, minimum is 1", nBands);
		cerr << "error in ofSoundSpectrum, you've asked for " << nBands << " bands, minimum is 1" << endl;
		...
		for(int i = 0; i < nBandsToGet; i++){
			...
			if (i >= ((currentBand+1)*step)){
				...
				if (currentBand >= nBands){
					...
					cerr << "ofSoundGetSpectrum - currentBand >= nBands" << endl;
				}
...
void ofSoundPlayer::loadSound(string fileName, bool stream){
	...
	if (result != FMOD_OK){
		...
		cerr << "ofSoundPlayer: Could not load sound file " << fileName << endl;
	} else {
		bLoadedOk = true;
